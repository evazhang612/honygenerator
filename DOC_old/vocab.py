...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        padded_chars = pad_sents_char(self.words2charindices(sents), 0)
        x = torch.tensor(padded_chars, dtype=torch.long, device=device) # (batch_size, max_sentence_length, max_word_length)
        x = x.permute(1, 0, 2)

        return x.contiguous() # (max_sentence_length, batch_size, max_word_length)
